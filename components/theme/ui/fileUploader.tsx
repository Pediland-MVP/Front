...
import { File as FileIcon, MusicNote, X, UploadSimple } from "@phosphor-icons/react"
import { Button } from "@/components/ui/button"
import type { FileUploaderProps, FileWithPreview, UploadedFile } from "@/components/theme/types/fileUploader"
import { Pause, Play } from "@phosphor-icons/react/dist/ssr"

export function FileUploader({ multiple = false, files, setFiles, accept, onChange }: FileUploaderProps) {
  ...
  const removeFile = (id: number) => {
    const updatedFiles = files.filter((file): file is UploadedFile => "id" in file && file.id !== id)
    setFiles(updatedFiles)
    onChange(updatedFiles)
  }
  ...
  const renderPreview = (file: UploadedFile) => {
    ...
    const renderPlay = () => {
      ...
      return (
        <div className="absolute z-20 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          {isPlaying ? (
            <Pause onClick={handlePlay} size={48} fill="white" className="text-gray-300" />
          ) : (
            <Play onClick={handlePlay} size={48} fill="white" className="text-gray-300" />
          )}
        </div>
      )
    }
    ...
    switch (fileType) {
      case "image":
        return (
          <div className="w-full h-full object-cover relative">
            {"isUploading" in file && file.isUploading && renderUploadCircularProgress()}
            <img
      ...
      case "video":
        return (
          <div className="w-full h-full object-cover relative">
            {"isUploading" in file && file.isUploading ? renderUploadCircularProgress() : renderPlay()}
            <video
  ...
  return (
    <div className="w-full">
      <div {...getRootProps()} className="relative">
        <input {...getInputProps()} />
        <Button type="button" onClick={open} className="w-full flex items-center justify-center gap-2">
      ...
      <div className="flex justify-center items-start flex-col mt-2">
        ...
        <span className="text-xs text-gray-500">{t("Limits.media.formats")}</span>
      </div>
    </div>
  )
}
