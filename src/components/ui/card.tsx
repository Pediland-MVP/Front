...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-2.5 rounded-xl border py-5 shadow-sm",
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "text-secondary flex items-center gap-1.5 text-[15px] leading-none font-semibold [&_svg]:size-5.5",
        className,
      )}
      {...props}
    />
