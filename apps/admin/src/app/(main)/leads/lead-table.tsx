...
export default function LeadTable({
  ...
  const [tableInstance, setTableInstance] = useState<Table<MarketingLead> | null>(null);
  ...
  const handleExportLeads = async (data: {
    ...
    try {
      ...
      if (response.data) {
        toast.success(
          'درخواست خروجی اکسل سرنخ‌ها با موفقیت ارسال شد. لطفاً ایمیل خود را بررسی کنید.',
        );
      }
    } catch (error) {
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-hidden p-4">
        <div className="flex flex-wrap items-center gap-2 pb-3">
          <div className="grid flex-1 grid-cols-2 items-center gap-1.5 md:flex">
            ...
            <FilterStatus type="lead" value={leadsStatus} onChange={onStatusChange} />

            {user && user.role !== 'kam' && (
              <FilterAdmin data={kams} value={leadAdmins} onChange={onAdminChange} />
            )}
