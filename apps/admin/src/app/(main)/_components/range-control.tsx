...
import { RANGE_DAY_PRESETS, type RangeConfig, type RangeDays } from './metrics.constants';
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {RANGE_DAY_PRESETS.map((days: RangeDays) => (
        <Button
          key={days}
          size="sm"
          variant={value.mode === 'preset' && value.days === days ? 'default' : 'outline'}
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button size="sm" variant={value.mode === 'custom' ? 'default' : 'outline'}>
            <CalendarRange className="size-4" />
            {value.mode === 'custom' ? `${fmt(value.from)} – ${fmt(value.to)}` : t('customRange')}
          </Button>
        </PopoverTrigger>
