...
export default function QAPage() {
  ...
  useEffect(() => { fetchQa(page); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [page]);

  const handleAddQa = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!question.trim() || !answer.trim()) { toast.error("لطفاً هم پرسش و هم پاسخ را تکمیل کنید"); return; }
    setSaving(true);
    try {
  ...
  const startEdit = (item: QaType) => {
    setEditingId(item.id); setEditQuestion(item.question); setEditAnswer(item.answer);
  };
  const cancelEdit = () => { setEditingId(null); setEditQuestion(""); setEditAnswer(""); };

  const handleUpdateQa = async (id: string) => {
    if (!editQuestion.trim() || !editAnswer.trim()) { toast.error("پرسش و پاسخ نمی‌توانند خالی باشند"); return; }
    setUpdatingId(id);
    try {
      const { data } = await api.put(`/telegram-automation/qa/${id}`, { question: editQuestion, answer: editAnswer });
          ...
          <div className="lg:col-span-2 space-y-4 lg:space-y-0 lg:gap-4 lg:flex lg:flex-col lg:min-h-0">
            ...
            {loading ? (
              ...
              <div className="flex h-48 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white p-6 text-center">
                ...
                <p className="text-sm font-medium text-slate-500">هنوز هیچ پرسش و پاسخی ثبت نشده است</p>
                <p className="text-xs text-slate-400 mt-1">از فرم سمت راست برای تعریف اولین مورد استفاده کنید</p>
              </div>
            ) : (
              <div className="space-y-4">
                {qaList.map((item) =>
                  editingId === item.id ? (
                    <div key={item.id} className="rounded-2xl border border-blue-200 bg-blue-50/50 p-5 shadow-xs">
                    ...
                    <div key={item.id} className="group relative rounded-2xl border border-slate-100 bg-white p-5 shadow-xs transition-shadow hover:shadow-sm flex gap-4 items-start">
                      <div className="flex-1 space-y-2">
                        ...
                        <div className="text-sm text-slate-600 flex items-start gap-2 pr-1">
