...
export function SelectAdmins({
  ...
  return (
    <div className="order-1 col-span-2 flex items-center gap-1.5">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button role="combobox" className="flex-1 justify-between md:w-[140px]">
            {selectedKam ? `${selectedKam.firstname} ${selectedKam.lastname}` : 'انتخاب کنید'}
                ...
                {kams.map((kam) => {
                  ...
                  return (
                    <CommandItem key={kam.id} value={kam.id} onSelect={() => handleSelect(kam.id)}>
                      {fullName}
                      <Check
                        className={cn('mr-auto', value === kam.id ? 'opacity-100' : 'opacity-0')}
                      />
                    </CommandItem>
                  );
                })}
      ...
      <Button icon onClick={handleAssignAdmin}>
        <CheckIcon />
      </Button>
    </div>
  );
