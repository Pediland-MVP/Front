...
interface OrderCardComponentProps {
  ...
  setSelectedOrder: React.Dispatch<React.SetStateAction<OrderNamespace.GET.OneItemOfOrders | null>>;
}
  ...
  return (
    <Card className="gap-0 border-violet-200 p-0 shadow-violet-200">
        ...
        <div className="flex gap-1.5">
          <div className="flex flex-col gap-2">
            <div className="relative h-20 w-20">
              <CardImage
                src={order.orderProducts[0]?.product?.images[0]?.url || '/images/placeholder.webp'}
            ...
            <div className="text-secondary flex flex-1 justify-center text-[13px]">
              ...
              >
                {order.orderProducts?.[0]?.product?.isDigital ? t('digital') : t('physical')}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex flex-1 flex-col space-y-2 overflow-hidden p-1 text-[13px]">
            ...
            <div className="flex items-center gap-1">
              <span className="text-muted-foreground truncate">تاریخ سفارش:</span>
              <span className="text-secondary">{toJalaliDate(order.createDate)}</span>
            </div>
            <div className="flex items-center gap-1">
            ...
            <div className="text-secondary flex items-center gap-1">
              ...
              <div>
                {order.paymentMethod === 'card_to_card' ? t('card_to_card') : t('zarinpal')}
              </div>
            </div>
            <div className="flex items-center gap-1">
      ...
      <CardFooter className="flex rounded-b-xl bg-gray-100 p-0">
        <Button
          className={cn('h-9 w-full flex-1 cursor-auto gap-1 rounded-none rounded-br-xl', bg, text)}
          variant="ghost"
          type="button"
          size="sm"
        >
