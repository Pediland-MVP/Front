...
// repo's other Select tests (BusinessInfoDialog.test.tsx) sidestep it the same way: swap
// in a native <select> so the province/city fields render without blowing up.
...
// Keyed by SWR key so each account's GET response can be driven independently, the same
// pattern WelcomeMessageManager.test.tsx uses. A key absent from the map behaves like a
// request that hasn't resolved yet (data: undefined) — distinct from a key present with
// `data: null`, which is what a real account with no saved address returns.
...
const mockShopAddress = (instagramId: string, address: Record<string, unknown> | null) => {
  swrResponses[shopAddressKey(instagramId)] = {
    data: { message: 'OK', statusCode: 200, code: 'X', data: address },
  };
};

const renderDialog = (instagramId: string) =>
  render(
    <NextIntlClientProvider locale="fa" messages={messages}>
      <ShopAddressDialog instagramId={instagramId} open onOpenChange={() => {}} canManage />
    </NextIntlClientProvider>,
  );

const rerenderDialog = (rerender: (ui: React.ReactElement) => void, instagramId: string) =>
  rerender(
    <NextIntlClientProvider locale="fa" messages={messages}>
      <ShopAddressDialog instagramId={instagramId} open onOpenChange={() => {}} canManage />
    </NextIntlClientProvider>,
...
describe('ShopAddressDialog', () => {
  ...
  it('disables every field (not just the save button) when canManage is false, so read-only is actually read-only', () => {
    mockShopAddress('ig-a', {
      id: 'sa-a',
      address: 'آدرس',
      postalcode: '1111111111',
    ...
    render(
      <NextIntlClientProvider locale="fa" messages={messages}>
        <ShopAddressDialog instagramId="ig-a" open onOpenChange={() => {}} canManage={false} />
      </NextIntlClientProvider>,
    );
