...
/**
 * Same handshake as IG_OAUTH_URL, but stamps `state=reconnect` — Instagram passes `state`
 * back untouched, so the backend's `redirectToFrontend` route can tell /connect this code is
 * refreshing an already-connected account's token, not adding a new one. Every "relogin"
 * surface (invalid-token banner, settings reconnect dialog) must use this, not IG_OAUTH_URL,
 * or /connect falls back to treating the relogin as "add another Instagram".
 */
export const IG_RECONNECT_OAUTH_URL = `${IG_OAUTH_URL}&state=reconnect`;
