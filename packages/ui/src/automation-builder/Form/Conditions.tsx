...
// dashboard-side. Inlined here (instead of moving that whole types module) since
// `ConditionTypesEnum` right below already encodes the same three values and this is the
// only place that needed the type name.
type ContentCycleConditionTypes = 'EQUAL' | 'INCLUDE' | 'noCondition';

type ConditionsProps = {
  control: Control<AutomationFormType>;
  getValues: UseFormGetValues<AutomationFormType>;
  /** Rendered next to the condition-type select. Replaces the dashboard-only
...
export const Conditions = ({ control, getValues, helpSlot }: ConditionsProps) => {
            ...
            render={({ field }) => {
              return (
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={ConditionTypesEnum.EQUAL}>{t('equal')}</SelectItem>
                    <SelectItem value={ConditionTypesEnum.INCLUDE}>{t('include')}</SelectItem>
      ...
      {conditionTypeWatch !== ConditionTypesEnum.NO_CONDITION && (
        <div className="space-y-2">
          {conditionsField.map((condition, index) => (
            <div key={condition._xid} className="flex flex-col items-start gap-2 xl:flex-row">
