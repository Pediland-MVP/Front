    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          ...
          <div className="flex h-full w-full flex-col gap-4 px-3 pt-5 pb-3">{children}</div>
        </SheetContent>
      </Sheet>
    );
  }
    ...
    >
      ...
      >
        <div
          ...
          className="flex h-full w-full flex-col gap-4 rounded-xl bg-linear-to-t from-white/85 to-violet-50 p-3 pt-4 group-data-[variant=floating]:rounded-xl group-data-[variant=floating]:border group-data-[variant=floating]:border-blue-800/20"
        >
          {children}
        </div>
      </div>
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      variant="link"
      size="icon"
      className={cn('md:text-secondary hover:text-primary w-auto text-white', className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
function SidebarMenuAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        'ring-sidebar-ring peer-hover/menu-button:text-sidebar-accent-foreground group-hover/menu-item:[&>svg]:text-primary group-data-[state=open]/menu-item:[&>svg]:text-primary text-secondary absolute top-0 left-1.5 flex aspect-square w-5 cursor-pointer items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-3.5 [&>svg]:shrink-0',
        // Increases the hit area of the button on mobile.
        'after:absolute after:-inset-2 md:after:hidden',
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return (
    <ul
      ...
      className={cn(
        'flex min-w-0 translate-x-px flex-col gap-1.5 px-8 pt-1.5',
        'group-data-[collapsible=icon]:hidden',
        className,
      )}
      {...props}
