...
| `apps/dashboard/src/components/Settings/ShopAddressDialog.tsx` | `GET`/`PUT /instagram/:instagramID/shopAddress` | **New 2026-09-09**, one `ShopAddress` row per Instagram account (province/city/address/postal code/phone/shipping method) — the seller's own sender identity printed on shipping labels/invoices via `IShopAddress` (`types/instagram/shopAddress.ts`). Opened from a 4th action button on `InstagramAccounts.tsx`, kept enabled for `instagram:view-only` users (fields/save disabled instead) so a read-only user can still see the saved address. `PUT` errors `SHOP_ADDRESS_INVALID_CITY` on a bad `cityId` (translated in `messages/fa/ErrorCodes.json`). Back doc: `Back/knowledge/updates/2026-09-09-orderPrintDocumentsCommerce.update.md`. |
...
| `GET /commerce/orders/:id` | `apps/dashboard/src/hooks/useCommerceOrder.ts`, consumed by `OrderDetail.tsx`/`OrderSummaryRail.tsx` | `OrderDetailView` (`OrderView` + `receipts: OrderReceiptView[]`, full trail newest-first). **As of 2026-09-09**, also carries `shop: OrderShopView \| null` (detail only, not the list route) — the seller's Instagram/`ShopAddress` identity, resolved server-side from `customer.instagramId`, consumed by `OrderDetail.tsx`'s print-label/print-invoice buttons as the document's sender block. `null` when the order predates the `commerce_customer.instagramId` backfill or the account has no saved `ShopAddress`. Back doc: `Back/knowledge/updates/2026-09-09-orderPrintDocumentsCommerce.update.md`. |
