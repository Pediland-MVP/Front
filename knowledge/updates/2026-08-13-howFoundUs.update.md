...
- `apps/admin/src/messages/fa.json` has a **duplicate top-level `Jobs` key**. The second
  wins at runtime, so the first block is dead. Do not round-trip that file through
  `JSON.parse`/`stringify` — it silently drops one copy. Edit it as text.
- `packages/ui/node_modules/next-intl`, a pnpm symlink, is tracked in git, so any
  `pnpm install` in a worktree shows it as modified.
