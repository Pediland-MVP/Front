...
Until a post is picked, the URL input stays hidden and only a fullsize "select post" button is shown (so the user isn't looking at an empty/irrelevant URL field). Once a post is picked, the field flips back to the normal layout: the URL input plus a small icon button next to it (to change the pick).
...
- `packages/ui/src/automation-builder/Contents/ContentButtonsItem.tsx`: new button-type option, per-row local "displayed type" state (since the saved form value alone can't distinguish "Instagram Post" from plain "URL" once saved), guard check + dialog wiring. The url `FormField`'s render now branches on whether the field already has a value: empty → fullsize picker button only (input hidden); has a value → input + small icon button (original layout).
...
## Verification
- Unit tests: `packages/ui/src/automation-builder/Form/__tests__/InstagramPostSelectDialog.test.tsx`, `packages/ui/src/automation-builder/Contents/__tests__/ContentButtonsItem.test.tsx` (5/5 pass, including the fullsize/compact toggle covered indirectly by the existing pick and type-switch tests).
