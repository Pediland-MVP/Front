...
export function ContentsUploaderContextProvider({ children, defaultValue }: { children: React.ReactNode, defaultValue: UploadedFile }) {
...
export function useContentsUploaderContext() {
    const context = useContext(ContentsUploaderContext);
    if (context === null) {
        throw new Error("useContentsUploaderContext must be used within a ContentsUploaderContextProvider");
    }
    return context;
}
