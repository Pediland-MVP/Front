...
function SortableItem({ 
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style}
      className="border-[1.2px] p-2 rounded-2xl flex gap-x-4 items-center"
    >
      <div 
        {...attributes} 
        {...listeners} 
...
export default function Questions({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      const oldIndex = questionsField.findIndex(field => field._xid === active.id);
      const newIndex = questionsField.findIndex(field => field._xid === over?.id);
