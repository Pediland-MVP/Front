...
export function DrawerDialogDemo({ open, setOpen, facebookAccountId }: DrawerDialogDemoProps) {
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <SelectPagesForm facebookAccountId={facebookAccountId} />
        </DialogContent>
      </Dialog>
    );
  }
  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerContent>
        ...
        <SelectPagesForm facebookAccountId={facebookAccountId} className="px-4" />
...
function SelectPagesForm({ className, facebookAccountId }: {className?: string, facebookAccountId: string}) {
  ...
  const handleSelectInstagram = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent> ) => {
    e.preventDefault();
    const pageId = e.currentTarget.dataset.id
    setLoading({id: pageId!})
    await fetch(`${process.env.NEXT_PUBLIC_BACK_API_URL}/instagram/select/${facebookAccountId}/${pageId}`, {
        method: 'PATCH',
        credentials: 'include'
    })
    .then(async res => {
        ...
        if (!res.ok) {
            ...
            toast({
                title: isExist ? "این اکانت قبلا انتخاب شده است" : "ارتباط با سرور برقرار نشد",
                description: isExist ? "اکانت دیگری انتخاب کنید" : "ارتباط با سرور برقرار نشد",
                variant: "destructive",
            })
            return;
        }
  ...
  return (
    <form className={cn(className)}>
        ...
        {Array.isArray(instagramPages) && instagramPages?.map((instagram) => {
            ...
            >
                {instagram.profile_picture_url ? <Image className="rounded-full" src={instagram.profile_picture_url} width={70} height={70} alt={instagram.name} /> : <InstagramLogo size={70} />}
                <p>{instagram.name}</p>
                <p>{instagram.username}</p>
                <Button onClick={handleSelectInstagram} data-id={instagram.id} className="w-full text-black flex justify-center items-center gap-x-2" variant={'outline'}>
                    انتخاب
                    {
